...
func _init():
	super("Geluid en muziek (UNDER CONSTRUCTION)", [
		...
		TutorialStep.new("Afspelen vanuit code 1", 
			"Vanuit code speel je een geluid af met de 'play()' functie van de AudioStreamPlayer.\n\nVoeg een script toe aan je root Node2D."),
		TutorialStep.new("Afspelen vanuit code 2", 
			"Gebruik het volgende script:\n\t\textends Node2D\n\n\t\tfunc _ready():\n\t\t\t$AudioStreamPlayer.play()"),
		TutorialStep.new("Afspelen vanuit code 3", 
			"Om dit te testen moet je de huidige scene opslaan.\n\nDan kan je de huidige scene runnen met F6.\n\nProbeer het eens!",
			null, Callable(),
...
func _has_added_child_audioStreamPlayer_node() -> bool:
	...
	return EditorInterface.get_edited_scene_root().get_children().filter(func(node): return node is AudioStreamPlayer).size() == 1
...
func _audio_stream_player_has_stream() -> bool:
	return EditorInterface.get_edited_scene_root().get_children().any(func(node): return node is AudioStreamPlayer and node.stream != null)
